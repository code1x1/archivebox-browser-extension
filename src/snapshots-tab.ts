...
export function initializeSnapshotsTab() {
    ...
    function getCurrentModalTags() {
        return Array.from(
            document
                .getElementById('currentTagsList')
    ...
    async function updateCurrentTagsList() {
        const { entries: snapshots = [] } =
            await browser.storage.local.get('entries')
        const selectedSnapshotsArray = snapshots.filter((e) =>
            selectedSnapshots.has(e.id)
        )
        ...
        const commonTags = selectedSnapshotsArray.reduce((acc, snapshot) => {
            ...
            return new Set(
                [...acc].filter((tag) => snapshot.tags.includes(tag))
            )
        }, null)
    ...
    async function renderSnapshots() {
      ...
      `
        ...
        snapshotsList.innerHTML = filteredSnapshots
            .map(
                (snapshot) => `
      <div>
        <input type="checkbox" 
        ...
        <div>
          <div>
            <div>${snapshot.title || 'Untitled'}</div>
            ${(() => {
                return archivebox_server_url
                   ...
                   <a href=${archivebox_server_url}/archive/${snapshot.url} target="_blank">
                     📦 ArchiveBox
                   </a>
                   <a href="https://web.archive.org/web/${snapshot.url}" target="_blank">
                     🏛️ Archive.org
                   </a>
                 </div>`
                    : ''
          ...
          <div>
            <img src="${snapshot.favIconUrl || '128.png'}"
          ...
          <div>
            ${snapshot.tags.map((tag) => `<span>${tag}</span>`).join('')}
          </div>
        </div>
      </div>
    `
    ...
    document
        ...
        .addEventListener('click', async () => {
            const { entries: snapshots = [] } =
                await browser.storage.local.get('entries')
            const selectedItems = snapshots.filter((e) =>
                selectedSnapshots.has(e.id)
            )
    ...
    document
        ...
        .addEventListener('click', async () => {
            ...
            console.log(
                `deleting ${selectedItems.length} items from local storage`
            )
            ...
            await browser.storage.local.set({ entries: remainingSnapshots })
    ...
    document
        ...
        .addEventListener('click', async () => {
            ...
            console.log(
                `syncing ${selectedItems.length} items to ArchiveBox server`
            )
            ...
            syncBtn.innerHTML =
                '<span role="status" aria-hidden="true"></span> Syncing...'
            ...
            for (const item of selectedItems) {
                ...
                if (!row) continue
                const snapshot =
                    row.parentElement.querySelector('.snapshot-title')
                ...
                statusIndicator.style.backgroundColor = success
                    ? '#28a745'
                    : '#dc3545'
