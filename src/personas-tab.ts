...
async function detectCurrentSettings(personaId) {
    ...
    const settings = {
        ...
        timezone:
            persona.settings.timezone ||
            Intl.DateTimeFormat().resolvedOptions().timeZone,
        viewport:
            persona.settings.viewport ||
  ...
  `
    ...
    <tr data-id="${p.id}">
      ...
      <td>
        ...
        ${Object.keys(p.cookies || {})
            .map(
                (domain) =>
                    `<button data-persona-id="${p.id}" data-persona-domain="${domain}">${domain} ❌</button>`
            )
            .join(' ')}
      </td>
      ...
      <td>
        <div>
          ...
          <button data-id="${p.id}" style="display: none;">
            ✅ Save Changes
          </button>
          <button data-id="${p.id}">
            ❌ Delete Profile
...
async function updatePersonaStats(personaId) {
    ...
    const cookieCount = Object.values(persona.cookies || {}).reduce(
...
async function exportPersonaCookies(id) {
    ...
    alert(
        `${Object.keys(persona.cookies).length} domain logins (${Object.values(persona.cookies).reduce((sum, cookies) => sum + cookies.length, 0)} cookies) copied to clipboard for "${persona.name}"! Save them into cookies.txt on your ArchiveBox server and run: archivebox config --set COOKIES_FILE=/path/to/cookies.txt`
    )
}
...
export function initializePersonasTab() {
    // Persona management
    document
        .getElementById('newPersona')
        .addEventListener('click', createNewPersona)
    document
        .getElementById('activePersona')
        .addEventListener('change', (e) => setActivePersona(e.target.value))

    document
        .getElementById('personaTable')
