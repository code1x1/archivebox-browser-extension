...
export async function loadAvailableCookies() {
    ...
    availableCookies = Object.entries(cookiesByDomain).map(
        ([domain, cookies]) => ({
            domain,
            cookies,
...
function updateSelectedCount() {
    ...
    document.getElementById('importCookies').disabled =
        count === 0 || !document.getElementById('activePersona').value
}

function toggleAllCookieSelection(selected) {
    const filterText = document
        .getElementById('cookieFilter')
        .value.toLowerCase()
...
async function previewCookies(domain) {
    const cookies =
        availableCookies.find((item) => item.domain === domain)?.cookies || []
    const text = formatCookiesForExport({ [domain]: cookies })
    await navigator.clipboard.writeText(text)
    alert(
        `${cookies.length} cookies copied to clipboard for "${domain}"!  Save them into cookies.txt on your ArchiveBox server and run: archivebox config --set COOKIES_FILE=/path/to/cookies.txt`
    )
}
...
async function importSelectedCookies() {
    ...
    alert(
        `Successfully imported ${importCount} domain cookies into the "${persona.name}" persona`
    )
...
export function initializeCookiesTab() {
    ...
    document
        .getElementById('cookieFilter')
        .addEventListener('input', (e) => renderCookieTable(e.target.value))
    ;['selectAllCookies', 'selectAllCookiesBottom'].forEach((id) =>
        document
            .getElementById(id)
            .addEventListener('click', () => toggleAllCookieSelection(true))
    )
    ;['deselectAllCookies', 'deselectAllCookiesBottom'].forEach((id) =>
        document
            .getElementById(id)
            .addEventListener('click', () => toggleAllCookieSelection(false))
    )

    document
        .getElementById('cookieTable')
    ...
    document
        .getElementById('importCookies')
        .addEventListener('click', importSelectedCookies)
}
